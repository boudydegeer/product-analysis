...
class AnalysisOverviewResponse(BaseModel):
    ...
    metrics: dict[str, Any] = Field(
        default_factory=dict, description="Complexity, effort, confidence"
    )
...
class ArchitectureInfo(BaseModel):
    ...
    components: list[str] = Field(
        default_factory=list, description="List of components"
    )
...
class AnalysisImplementationResponse(BaseModel):
    ...
    architecture: dict[str, Any] = Field(
        default_factory=dict, description="Architecture details"
    )
    technical_details: list[dict[str, Any]] = Field(
        default_factory=list, description="Technical implementation details"
    )
    data_flow: dict[str, Any] = Field(
        default_factory=dict, description="Data flow information"
    )
...
class AnalysisRisksResponse(BaseModel):
    ...
    technical_risks: list[dict[str, Any]] = Field(
        default_factory=list, description="Technical risks"
    )
    security_concerns: list[dict[str, Any]] = Field(
        default_factory=list, description="Security issues"
    )
    scalability_issues: list[dict[str, Any]] = Field(
        default_factory=list, description="Scalability concerns"
    )
...
class AnalysisDetailResponse(BaseModel):
    ...
    status: Literal["completed", "no_analysis", "failed", "analyzing"] = Field(
